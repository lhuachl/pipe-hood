...
import { IExecutor } from '../src/types/index.js';
...
class LoggingExecutor implements IExecutor {
  constructor(private baseExecutor: IExecutor) {}

  async execute<T = any>(sql: string, params: unknown[]): Promise<T[]> {
    console.log('🔍 Ejecutando SQL:', sql);
    console.log('📦 Parámetros:', params);
    return this.baseExecutor.execute<T>(sql, params);
  }
}
...
async function main() {
  try {
    ...
    // Crear base executor
    const { SupabaseExecutor } = await import(
      '../src/executor/SupabaseExecutor.js'
    );
